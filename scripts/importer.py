...
def create_object(level, data):
    ...
    portal_type = json_data['_type']
    portal_type = PT_REPLACE_MAP.get(portal_type, portal_type)
    ...
    if portal_type == "Folder" and id != "carousel":
        print json_file,"*****"
        create_folder(**args)
    elif portal_type == "Collection":
        create_collection(**args)
    elif portal_type in ["News Item", "leader", "feature"]:
        create_news_item(**args)
    elif portal_type in ["Document", "blog_entry"]:
        create_document(**args)
    elif portal_type == "Link":
        create_link(**args)
    elif portal_type == "Image":
        create_image(**args)
    elif portal_type == "File":
        create_file(**args)

def create_folder(level, id, base_path, path_, name, data, portal_type="Folder"):
...
def create_collection(level, id, base_path, path_, name, data, **kwargs):
    container = api.portal.get()
    _path = base_path
    if level > 1:
        parent = data['_path'].split('/')[-2]
...
def create_document(level, id, base_path, path_, name, data, portal_type="Document"):
    container = api.portal.get()
    if level > 1:
        parent = data['_path'].split('/')[-2]
        _path = "/".join(base_path.split('/')[:-1])
...
def create_link(level, id, base_path, path_, name, data, **kwargs):
    container = api.portal.get()
    if level > 1:
        parent = data['_path'].split('/')[-2]
        _path = "/".join(base_path.split('/')[:-1])
...
def create_news_item(level, id, base_path, path_, name, data, portal_type="News Item"):
    otherdata = {
        "exclude_from_nav":True,
        "contributors":data["contributors"],
        "text":RichTextValue(data["text"]),
...
def create_file(level, id, base_path, path_, name, data, **kwargs):
...
def create_image(level, id, base_path, path_, name, data, **kwargs):
    if 'title' in data:
        name = data['title']
    else:
        print "{} has no title".format(json_file)
...
def set_dates(obj, data):
    ...
    obj.setExpirationDate(data.get('expirationDate'))
    obj.setEffectiveDate(data.get('effectiveDate'))
    obj.setModificationDate(data.get('modification_date'))
